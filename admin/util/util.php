...
/**
*
* Configuração de PATH da aplicação
*
*/
    ...
    foreach ($post as $key => $value) {
        if (is_array($value))
            ...
            if (get_magic_quotes_gpc())
                ...
                $post[$key] = addslashes(trim($post[$key]));
        }
    }
}
...
function Erro2($str) {
    $_SESSION['Erros2'][isset($_SESSION['Erros2']) ? count($_SESSION['Erros2']):1] = $str;
}
...
function showErros2() {
    $erros = isset($_SESSION['Erros2']) ? count($_SESSION['Erros2']):0;
    
    if ($erros > 0) {
        echo '<div class="alert alert-error">
                <button class="close" data-dismiss="alert">×</button>';

        foreach ($_SESSION['Erros2'] as $erro) {
            echo '<p>' . $erro . '</p>';
        }
        echo '</div>';        
        unset($_SESSION['Erros2']);
    }    
}
...
{    
    ...
    $info = isset($_SESSION['Info']) ? count($_SESSION['Info']):0;
    ...
    if ($info > 0)
    {
        echo '<div class="alert alert-success"><button class="close" data-dismiss="alert">x</button>' . $_SESSION['Info'] . '</div>';
        unset($_SESSION['Info']);        
    }
}
...
function validarCPF($cpf) { // Verifiva se o número digitado contém todos os digitos
...
function ismyimage($myfile) {
    if ((($myfile["type"] == "image/gif") || ($myfile["type"] == "image/jpg") || ($myfile["type"] == "image/jpeg") || ($myfile["type"] == "image/png")) && ($myfile["size"] <= 2097152 /* 2mb */))
        return true;
    else
        return false;
}
